...
static sF32 Noise2(sInt x, sInt y, sInt maskx, sInt masky, sInt seed) {
  ...
  return LerpF(
      ...
      LerpF(
          ...
          (P(((X + 1) & maskx) + P(((Y + 0) & masky)) + seed)) / 2047.5f -
              1.0f),
      LerpF(
          u,
          (P(((X + 0) & maskx) + P(((Y + 1) & masky)) + seed)) / 2047.5f - 1.0f,
          (P(((X + 1) & maskx) + P(((Y + 1) & masky)) + seed)) / 2047.5f -
              1.0f));
}
...
static sF32 PNoise2(sInt x, sInt y, sInt maskx, sInt masky, sInt seed) {
  ...
  return S *
         LerpF(v,
               LerpF(u,
                     PGradient2(
                         (P(((X + 0) & maskx) + P(((Y + 0) & masky)) + seed)),
                         fx, fy),
                     PGradient2(
                         (P(((X + 1) & maskx) + P(((Y + 0) & masky)) + seed)),
                         fx - 1.0f, fy)),
               LerpF(u,
                     PGradient2(
                         (P(((X + 0) & maskx) + P(((Y + 1) & masky)) + seed)),
                         fx, fy - 1.0f),
                     PGradient2(
                         (P(((X + 1) & maskx) + P(((Y + 1) & masky)) + seed)),
                         fx - 1.0f, fy - 1.0f)));
}
  ...
  for (sInt oy = 0; oy <= 1; oy++) {
    for (sInt ox = 0; ox <= 1; ox++) {
      ...
      if (t < 1.0f) {
        ...
        t *= t;
        sum +=
            t * PGradient2(GShuffle((i + ox) & maskx, (j + oy) & masky, seed),
                           xr, yr);
      }
    }
  }
...
// ---- Sampling helpers
void GenTexture::SampleNearest(Pixel& result,
                               sInt x,
                               sInt y,
                               sInt wrapMode) const {
  if (wrapMode & 1)
...
void GenTexture::SampleBilinear(Pixel& result,
                                sInt x,
                                sInt y,
                                sInt wrapMode) const {
  if (wrapMode & 1)
...
void GenTexture::SampleFiltered(Pixel& result,
                                sInt x,
                                sInt y,
                                sInt filterMode) const {
...
void GenTexture::Noise(const GenTexture& grad,
                       sInt freqX,
                       sInt freqY,
                       sInt oct,
                       sF32 fadeoff,
  ...
  for (sInt y = 0; y < YRes; y++) {
    for (sInt x = 0; x < XRes; x++) {
      ...
      for (sInt i = 0; i < oct; i++) {
        sF32 nv = (mode & NoiseBandlimit) ? Noise2(px, py, mx, my, seed)
                                          : GNoise2(px, py, mx, my, seed);
        if (mode & NoiseAbs)
          nv = sFAbs(nv);
...
void GenTexture::GlowRect(const GenTexture& bgTex,
...
void GenTexture::Cells(const GenTexture& grad,
                       const CellCenter* centers,
                       sInt nCenters,
                       sF32 amp,
                       sInt mode) {
...
void GenTexture::ColorMatrixTransform(const GenTexture& x,
  ...
  for (sInt i = 0; i < NPixels; i++) {
    ...
    sInt r = MulShift16(m[0][0], in.r) + MulShift16(m[0][1], in.g) +
             MulShift16(m[0][2], in.b) + MulShift16(m[0][3], in.a);
    sInt g = MulShift16(m[1][0], in.r) + MulShift16(m[1][1], in.g) +
             MulShift16(m[1][2], in.b) + MulShift16(m[1][3], in.a);
    sInt b = MulShift16(m[2][0], in.r) + MulShift16(m[2][1], in.g) +
             MulShift16(m[2][2], in.b) + MulShift16(m[2][3], in.a);
    sInt a = MulShift16(m[3][0], in.r) + MulShift16(m[3][1], in.g) +
             MulShift16(m[3][2], in.b) + MulShift16(m[3][3], in.a);
...
void GenTexture::CoordMatrixTransform(const GenTexture& in,
                                      const Matrix44& matrix,
                                      sInt mode) {
...
void GenTexture::ColorRemap(const GenTexture& inTex,
                            const GenTexture& mapR,
                            const GenTexture& mapG,
                            const GenTexture& mapB) {
...
void GenTexture::CoordRemap(const GenTexture& in,
                            const GenTexture& remapTex,
                            sF32 strengthU,
                            sF32 strengthV,
                            sInt mode) {
...
void GenTexture::Derive(const GenTexture& in, DeriveOp op, sF32 strength) {
  ...
  for (sInt y = 0; y < YRes; y++) {
    for (sInt x = 0; x < XRes; x++) {
      sInt dx2 = in.Data[y * XRes + ((x + 1) & (XRes - 1))].r -
                 in.Data[y * XRes + ((x - 1) & (XRes - 1))].r;
      sInt dy2 = in.Data[x + ((y + 1) & (YRes - 1)) * XRes].r -
                 in.Data[x + ((y - 1) & (YRes - 1)) * XRes].r;
...
static void Blur1DBuffer(Pixel* dst,
                         const Pixel* src,
                         sInt width,
                         sInt sizeFixed,
                         sInt wrapMode) {
  ...
  for (sInt x = 0; x < width; x++) {
    ...
    accu[0] += 64 * (src[xr0].r - src[xl1].r) +
               frac * (src[xr1].r - src[xr0].r - src[xl0].r + src[xl1].r);
    accu[1] += 64 * (src[xr0].g - src[xl1].g) +
               frac * (src[xr1].g - src[xr0].g - src[xl0].g + src[xl1].g);
    accu[2] += 64 * (src[xr0].b - src[xl1].b) +
               frac * (src[xr1].b - src[xr0].b - src[xl0].b + src[xl1].b);
    accu[3] += 64 * (src[xr0].a - src[xl1].a) +
               frac * (src[xr1].a - src[xr0].a - src[xl0].a + src[xl1].a);
  }
}

void GenTexture::Blur(const GenTexture& inImg,
                      sF32 sizex,
                      sF32 sizey,
                      sInt order,
                      sInt wrapMode) {
...
void GenTexture::Ternary(const GenTexture& in1Tex,
                         const GenTexture& in2Tex,
                         const GenTexture& in3Tex,
                         TernaryOp op) {
  sVERIFY(SizeMatchesWith(in1Tex) && SizeMatchesWith(in2Tex) &&
          SizeMatchesWith(in3Tex));
...
void GenTexture::Paste(const GenTexture& bgTex,
                       const GenTexture& inTex,
                       sF32 orgx,
                       sF32 orgy,
                       sF32 ux,
                       sF32 uy,
  ...
  // calculate bounding rect
  sInt minX =
      sMax<sInt>(0, floor((orgx + sMin(ux, 0.0f) + sMin(vx, 0.0f)) * XRes));
  sInt minY =
      sMax<sInt>(0, floor((orgy + sMin(uy, 0.0f) + sMin(vy, 0.0f)) * YRes));
  sInt maxX = sMin<sInt>(XRes - 1,
                         ceil((orgx + sMax(ux, 0.0f) + sMax(vx, 0.0f)) * XRes));
  sInt maxY = sMin<sInt>(YRes - 1,
                         ceil((orgy + sMax(uy, 0.0f) + sMax(vy, 0.0f)) * YRes));
  ...
  for (sInt y = minY; y <= maxY; y++) {
    ...
    for (sInt x = minX; x <= maxX; x++) {
      if (u >= 0 && u < 0x1000000 && v >= 0 && v < 0x1000000) {
        ...
        inTex.SampleFiltered(
            in, u, v,
            ClampU | ClampV | ((mode & 1) ? FilterBilinear : FilterNearest));
        ...
        switch (op) {
          ...
          case CombineMultiply:
            ...
            out->r = MulIntens(transIn, out->r) + MulIntens(transOut, in.r) +
                     MulIntens(in.r, out->r);
            out->g = MulIntens(transIn, out->g) + MulIntens(transOut, in.g) +
                     MulIntens(in.g, out->g);
            out->b = MulIntens(transIn, out->b) + MulIntens(transOut, in.b) +
                     MulIntens(in.b, out->b);
          ...
          case CombineDarken:
            out->r +=
                in.r - sMax(MulIntens(in.r, out->a), MulIntens(out->r, in.a));
            out->g +=
                in.g - sMax(MulIntens(in.g, out->a), MulIntens(out->g, in.a));
            out->b +=
                in.b - sMax(MulIntens(in.b, out->a), MulIntens(out->b, in.a));
            out->a += MulIntens(in.a, 65535 - out->a);
            break;

          case CombineLighten:
            out->r +=
                in.r - sMin(MulIntens(in.r, out->a), MulIntens(out->r, in.a));
            out->g +=
                in.g - sMin(MulIntens(in.g, out->a), MulIntens(out->g, in.a));
            out->b +=
                in.b - sMin(MulIntens(in.b, out->a), MulIntens(out->b, in.a));
            out->a += MulIntens(in.a, 65535 - out->a);
            break;
        }
      }
...
void GenTexture::Bump(const GenTexture& surface,
                      const GenTexture& normals,
                      const GenTexture* specular,
                      const GenTexture* falloffMap,
                      sF32 px,
                      sF32 py,
                      sF32 pz,
                      sF32 dx,
  ...
  for (sInt y = 0; y < YRes; y++) {
    for (sInt x = 0; x < XRes; x++) {
      ...
      if (!directional) {
        ...
        if (specular) {
          sF32 scale =
              sFInvSqrt(2.0f + 2.0f * L[2]);  // 1/sqrt((L + <0,0,1>)^2)
          H[0] = L[0] * scale;
          H[1] = L[1] * scale;
          H[2] = (L[2] + 1.0f) * scale;
        }
...
void GenTexture::LinearCombine(const Pixel& color,
                               sF32 constWeight,
                               const LinearInput* inputs,
                               sInt nInputs) {
